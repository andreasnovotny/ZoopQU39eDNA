...
interpolate_rollmean <- function(
  ...
  if (dim == 2) {
    interp_x2 <-
      crossing(
        dplyr::tibble(
          x2 = seq(min(interp_x1$x2), max(interp_x1$x2), by = x2res)
        ),
...
index_RA <- function(
  ...
  output <- data %>%
    ...
    # 1. Replace NA
    mutate(
      "{{abundance}}" := ifelse(is.na({{ abundance }}), na.rep, {{ abundance }})
    ) %>%
    ...
    group_by({{ taxa }}) %>%
    reframe(
      RA = ifelse(RA <= quantile(RA, extreme.perc), RA, -1),
      {{ sample }},
      {{ abundance }},
      ...
    ) %>%
    ...
    group_by({{ taxa }}) %>%
    reframe({{ abundance }}, RA, RA_Index = RA / max(RA), {{ sample }}, ...) %>%
...
rarefy_sequence_depth <- function(
  data,
  sample,
  taxa,
  abundance,
...
veglist_SIMPER <- function(veglist, group = NULL) {
  ...
  if (length(simper_summary) > 1) {
    stop(
      "Error: More than two sample groups detected. Please run SIMPER without a grouping variable."
    )
  }
...
getSpeciesList <- function(data, type = "DNA") {
  ...
  List <- data %>%
    ...
    mutate(
      Taxa = ifelse(
        Taxa == "Discoconchoecia aff. elegans CMARA05309_Os109.1.1",
