...
data <- eDNA_COI_indexed %>% 
  filter(Taxa == "Pseudocalanus minutus")
...
#  
  ...
  # Generalize datasets
  data <- data %>% 
    dplyr::transmute(x1 = Sample_date, x2 = Line_out_depth, y = RA_Index)
  
  # Execute first interpolation along x1
  interp_x1 <-
    tidyr::crossing(
      dplyr::tibble(x1 = seq(min(data$x1), max(data$x1), by = 1)),
...
```{r}
filter(Abundance > 0) %>% 
  interpolate_rollmean(Sample_date, Line_out_depth, y = RA_Index, x1res = 10, x2res = 10)
  ...
  ggplot() +
  geom_raster(aes(Sample_date, Line_out_depth, fill = RA_Index))
...
Interp_Species <- eDNA_COI_indexed %>%
  filter(Taxa %in% c(
    "Acartia",
    "Metridia pacifica",
  ...
  group_by(Taxa) %>%
  group_modify(~interpolate_rollmean(.x, x1 = Sample_date, x2 = Line_out_depth, y = RA_Index,
                                     # res 
