...
Good_18S_data <- Good_18S_data %>%
  mutate(Genus = ifelse(Genus == "Ditrichocorycaeus", "Corycaeus", Genus),
         Species = ifelse(Species == "Ditrichocorycaeus anglicus","Corycaeus anglicus", Species))
```
## TODO: CHECK 18S metadata components
...
calculate_index <- function(data, taxrank, project) {
  ...
  data %>% 
    ...
    mutate(Taxa = {{ taxrank }},
           ...
           Date = Sample_date,
           Sample = Library_ID) %>% 
    index_RA(Sample, Taxa, Abundance, Depth, Date, Hakai_ID)
}

eDNA_COI_index_species <- calculate_index(Good_COI_data, Species, "QU39")
eDNA_COI_index_genus <- calculate_index(Good_COI_data, Genus, "QU39")
