...
bottle_file %>% 
  pivot_longer(5:20, names_to = "Variable", values_to = "value") %>% 
...
```{r}
CTD_Interp <- CTD %>% 
  pivot_longer(7:14, names_to = "Parameter", values_to = "value") %>%
  filter(Parameter %in% c("Temperature", "Salinity")) %>% 
  group_by(Parameter) %>%
  group_modify(~interpolate_rollmean(.x, x1 = Depth, x2 = Date, y = value,
                                     x1res = 1, x2res = 1)) %>%
  filter(Depth < 250)
...
CTD_Interp  %>% 
  ggplot() +
  geom_point(aes(Date, Depth, color = value),
             filter(CTD_Interp, Parameter %in% c("Temperature"))) +
  scale_color_viridis(option = "H") +
  
  new_scale_color() +
  geom_point(aes(Date, Depth, color = value),
             filter(CTD_Interp, Parameter %in% c("Salinity")),) +
...
CHL %>%
  ...
  select(Date, Depth, filter_type, chla_final, phaeo_final) %>% 
  filter(filter_type == "Bulk GF/F") %>% 
  filter(Depth %in% c(0, 5, 30, 100)) %>% 
  ggplot() +
  geom_line(aes(Date, chla_final, color = Depth)) +
  #facet_wrap("Depth") +
  scale_x_date(date_labels="%b %y", breaks  = datebreaks)
...
CHL %>%
  ...
  filter(Depth %in% c(0, 5, 30, 100)) %>% 
  ggplot() +
  geom_line(aes(Date, phaeo_final, color = Depth)) +
  #facet_wrap("Depth") +
  scale_x_date(date_labels="%b %y", breaks  = datebreaks)
```
