...
## Filter taxa with to little data to be able to calculate distribution:

```{r}
# Plot distributions, arrange based on 

eDNA_COI_indexed %>% 
  filter(Abundance > 0) %>%
  group_by(Taxa) %>% 
  mutate(maxRA = max(RA)) %>%
  mutate(disper = quantile(RA_Index, 0.75)-quantile(RA_Index, 0.25)) %>% 
  ungroup() %>% 
  mutate(Taxa = fct_reorder(Taxa, -disper)) %>% 
  ggplot() +
  geom_boxplot(aes(Taxa, RA_Index)) +
...
useful_taxa <- eDNA_COI_indexed %>% 
  filter(Abundance > 0) %>%
  group_by(Taxa) %>% 
  summarise(disper = quantile(RA_Index, 0.75)-quantile(RA_Index, 0.25)) %>% 
  ungroup() %>% 
  filter(disper > 0) %>% 
  arrange(-disper) %>%
  mutate(Taxa = fct_reorder(Taxa, -disper)) %>%
...
```{r}
trait_summary_mod <- read_csv("Data_output/trait_summary_mod.csv") %>% 
  mutate(Taxa = scientificName)
...
eDNA_COI_interpolated %>%
  #mutate(Taxa = factor(Taxa, levels = useful_taxa)) %>%
  filter(Taxa %in% useful_taxa) %>% 
  ggplot() +
...
make species and trait matrixes:
...
```{r}
species_mod <- species_mat %>%
  t() %>% 
  metaMDS()

Site_scores <- scores(species_mod)$sites %>%
  as.data.frame() %>% 
  rownames_to_column("Sample")
Taxa_scores <- scores(species_mod)$species %>% 
  as.data.frame() %>% 
  rownames_to_column("Taxa")
```
...
```{r}
ggplot() +
   geom_text(data = Taxa_scores, aes(NMDS1, NMDS2, label = Taxa),
            size = 3, colour = "grey", fontface = "italic")
  #geom_point(data = Site_scores, aes(NMDS1, NMDS2), size = 3)
...
ggplot() +
   geom_text(data = Taxa_scores, aes(NMDS1, NMDS2, label = Taxa),
            size = 4, colour = "grey") +
geom_text(data = Site_scores, aes(NMDS1, NMDS2, label = Sample),
            size = 4, colour = "red", fontface = "italic")
  #geom_point(data = Site_scores, aes(NMDS1, NMDS2), size = 3)
...
kmeans(trait_mat, centers = 4)



best_trait <- useful::FitKMeans(trait_mat, max.clusters=20, nstart=25, seed=278613)
useful::PlotHartigan(best_trait)


trait_mat %>%
  as.data.frame() %>% 
  arrange(bodyLengthMax)
```
...
[1] "TG.omnivore"       "FM.passive"        "FM.ambush"         "FM.active"         "FM.current"        "FM.active.ambush"  "FM.cruise"        
 [8] "TG.herbivore"      "FM.passive.ambush" "RM.broadcasting"   "RM.brooding"       "TG.detritivore"    "RM.asexual"        "VD.epipelagic"    
[15] "VD.mesopelagic"    "VD.bathypelagic"   "TG.carnivore"      "MYE.absent"        "MYE.present"       "DVM.present"       "DVM.absent"       
[22] "DVM.reverse"       "DVM.weak"          "DVM.strong"        "HIB.present"       "BIO.present"       "BIO.absent"  
...
Trait_filt %>%
  filter(valueType == "categorical") %>%
  filter(traitName %in% c("feedingMode", "trophicGroup", "dielVerticalMigration")) %>% 
  select(scientificName, traitName, traitValue) %>% 
...
```{r}
Trait_filt %>% 
  filter(scientificName %in% useful_taxa) %>% 
  filter(traitName %in% c("trophicGroup", "bodyLengthMax", "feedingMode",
                          "dielVerticalMigration", "verticalDistribution")) %>% 
  select(scientificName, traitValue, traitName) %>%
  group_by(scientificName, traitName) %>% 
  mutate(traitValue = paste(traitValue, sep = "")) %>% 
  pivot_wider(names_from = traitName, values_from = traitValue) %>% 
  mutate_all(~paste(unique(.), sep = "")) %>% 
  transmute(functional_group = paste(feedingMode, trophicGroup),
            bodyLengthMax = as.numeric(bodyLengthMax),
            dielVerticalMigration,
            verticalDistribution) %>% 
  arrange(bodyLengthMax) %>% 
  write.csv("Data_output/trait_summary.csv")
